...
- `bootstrap/` 包含可运行的 Spring Boot 应用、认证/RBAC 接口、知识库接口、文档上传/解析/同步接口、摄入流水线（Pipeline + Task）接口、RAG 流式问答接口和对话记忆。
...
- `mcp-server/` 是 Spring Boot 骨架（默认端口 9099），目前仅有空 `DevBrainMcpServerApplication`，尚无业务实现。
- `frontend/` 包含 React 应用。认证/RBAC、后台知识库管理、文档管理、摄入流水线和 RAG 对话页面已调用真实 API；部分其他后台页面仍是路由占位。
