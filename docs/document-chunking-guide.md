...
> 本文档详细介绍 DevBrain 知识库管理系统中的 5 种文本分块策略，包括原理、适用场景、配置参数和选型建议。
...
DevBrain 提供 5 种分块策略，通过 `ChunkingStrategy` 接口统一抽象，由 `ChunkingStrategyFactory` 按模式索引。用户可在文档上传时指定策略，也可在分块配置中自定义参数。
